...
SetupProblem* SetupProblem::createFactory(std::string l_problem_type, std::string l_state_model){
    ...
    // Shocking 1T
    if ( !l_problem_type.compare("shocking") == true && 
         !l_state_model.compare("ChemNonEq1T") == true) 
    {
        return new SetupShocking1T();
    } 
    // Shocking TTv
    else if( !l_problem_type.compare("shocking") == true && 
             !l_state_model.compare("ChemNonEqTTv") == true) 
    {
        return new SetupShockingNT();
    } 
    // Larsen 1T
    else if( !l_problem_type.compare("larsen") == true && 
             !l_state_model.compare("ChemNonEq1T") == true) 
    {
    ...
    {
        std::cerr << "The requested type of problem has not "
